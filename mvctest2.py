...
class UnassignedCultistsView:
	...
	def __init__(self, frame, sizer, cult):
		self.cult = cult
...
class AllLaborPoolsView:
	...
	def __init__(self, frame, cult):	
		...
		self.unassigned_cultists_view = UnassignedCultistsView(frame, self.sizer, cult)
	...
	def update(self, cult):
		...
		for dept_name in cult.departments:
			if cult.departments[dept_name] not in self.labor_pool_watchers.keys():
				...
				self.labor_pool_watchers[cult.departments[dept_name]] = LaborPoolView(self.frame, self.sizer, cult.departments[dept_name], cult) #Adds itself to the window.
